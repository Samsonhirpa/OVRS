...
<form role="form" action="<?php echo base_url('ZoneSport/saveclub'); ?>" method="Post" enctype="multipart/form-data">
...
<div class="callout callout-info">
      ...
      <option value="">sport Fili</option>
      <?php
    foreach($sporttype as $row)
    {
     echo '<option value="'.$row->spt_id.'">'.$row->spt_name.'</option>';
    }
    ?>
  </select>
                  ...
                  <select id="clubtype" name="clubtype" class="form-control" >
...
$(document).ready(function(){
 $('#sporttype').change(function(){
  ...
  {
   $.ajax({
    url:"<?php echo base_url(); ?>Sport/fetch_clubtype",
    method:"POST",
    data:{spt_id:spt_id},
    success:function(data)
    {
     $('#clubtype').html(data);
    //  $('#clubtype').html('<option value="">Gosa kilabii Fil</option>');
    }
   });
  }
  else
  {
   $('#clubtype').html('<option value="">Gosa kilabii Fili</option>');
//   $('#subcity_woreda').html('<option value="">A/K/M Fili</option>');
  }
 });
