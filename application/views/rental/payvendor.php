...
<th class="sorting_asc" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" style="width: 90.95px;" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending">Withhold</th>
                     <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" style="width: 85.733px;" aria-label="Browser: activate to sort column ascending">Net Payable</th>
                 ...
                 <tbody>
                  ...
                  $no = 0;
                        ...
                        foreach ($this->str->getrent3() as $row ) {
                        $no++;
                        ?>
                    ...
                    <td>
                                     ...
                                     <a class="btn btn-xs alert-success" href="<?php echo base_url('structure/vendor_status/'.$row->id)?>"onclick="return confirm('Are you sure To is it Payed ??')"><i <?php if($row->vendor_status == 0 ) { ?> class="fa fa-cloud-upload">Not Payed </i> <i <?php } if($row->vendor_status == 1 ) { ?> class="fa fa-cloud-upload"> Reject </i> <i <?php } if($row->vendor_status == 2 ) { ?> class="fa fa-thumbs-o-up"><?php } ?>Payed</i> </a>
