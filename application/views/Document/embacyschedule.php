...
<div class="box box-default">
          <div class="box-header with-border">
            ...
            <i class="fa fa-plus-square btn btn-primary">ADD SCHEDULE</i>
      </button>
          </div>
          <div class="box-body">
...
<th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" style="width: 90.683px;" aria-label="Platform(s): activate to sort column ascending">OPERATOR</th>
                        ...
                        <tbody>
                              <?php
                              $no = 0;
                              foreach ($this->str->getschedule() as $row) {
                                    $no++;
                                    ?>
                                    <tr role="row" class="odd">
                                          ...
                                          <!-- <td><?php echo $row->permitfile; ?></td> -->
                                          <td> <img src="<?= base_url('upload/'.$row->permitfile) ?>" height="30px" width="60px">
                               </td> 
                                <td class="text-center">
                               <?php if(isset($row->embacydate) && strtotime($row->embacydate) <= strtotime(date('Y-m-d'))): ?>
                                    <span style="background-color:red;" class="badge badge-primary bg-gradient-primary px-3 rounded-pill"><?php echo $row->embacydate; ?></span>
                            <?php else: ?>
                               
                                  <span style="background-color:seagreen;" class="badge badge-success bg-success px-3 rounded-pill"><?php echo $row->embacydate; ?></span>
...
<td><a href="<?php echo base_url().'Structure/downloadschedule/'.$row->em_id; ?>" class="btn btn-success btn-sm"><span class="glyphicon glyphicon-download-alt"></a>
 <a class="btn btn-sm btn-danger" href="<?php echo base_url('Structure/Delete_Schedule/' . $row->em_id) ?>"onclick="return confirm('Are you sure To Delete ?')"><i class="fa fa-trash">Delete</i></a>
</td>
                                         <!--  <td>
